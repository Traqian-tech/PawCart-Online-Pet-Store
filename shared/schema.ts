...
export const wallets = pgTable('wallets', {
  ...
  totalEarned: decimal('totalEarned', { precision: 10, scale: 2 }).notNull().default('0'),
  totalSpent: decimal('totalSpent', { precision: 10, scale: 2 }).notNull().default('0'),
  frozenBalance: decimal('frozenBalance', { precision: 10, scale: 2 }).notNull().default('0'),
  createdAt: timestamp('createdAt').notNull().defaultNow(),
  updatedAt: timestamp('updatedAt').notNull().defaultNow(),
});
...
export const walletTransactions = pgTable('walletTransactions', {
  ...
  type: varchar('type', { length: 50 }).notNull(), // EARN, SPEND, REFUND, FREEZE, UNFREEZE
  source: varchar('source', { length: 100 }).notNull(), // GAME_FEED_PET, DAILY_CHECKIN, ORDER_PAYMENT, etc.
  ...
  description: text('description'),
  metadata: json('metadata'), // Extra info like game score, order ID, etc.
  createdAt: timestamp('createdAt').notNull().defaultNow(),
});

export const gameRecords = pgTable('gameRecords', {
  id: serial('id').primaryKey(),
  userId: integer('userId').notNull(),
  gameType: varchar('gameType', { length: 50 }).notNull(), // FEED_PET, MATCH_THREE, LUCKY_WHEEL, QUIZ
  score: integer('score').notNull().default(0),
  reward: decimal('reward', { precision: 10, scale: 2 }).notNull().default('0'),
  metadata: json('metadata'), // Game specific data
...
export const userTasks = pgTable('userTasks', {
  id: serial('id').primaryKey(),
  userId: integer('userId').notNull(),
  taskType: varchar('taskType', { length: 50 }).notNull(), // REVIEW_ORDER, SHARE_PRODUCT, REFER_FRIEND, etc.
