...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // First, get user email from session
      const { data: { session } } = await supabase.auth.getSession()
      let userEmail = session?.user?.email

      console.log('Resetting password for user:', userEmail || 'email will be retrieved after update')
      ...
      if (error) {
        ...
        // Handle specific error: new password same as old password
        if (error.message && error.message.includes('New password should be different from the old password')) {
          toast({
            title: 'Password Reset Failed',
            description: 'The new password must be different from your current password. Please choose a different password.',
            variant: 'destructive',
          })
        } else {
          toast({
      ...
      // Get user email from update response if not in session
      if (!userEmail) {
        userEmail = updateData?.user?.email
      }

      if (!userEmail) {
        // Try getting session again after update
        const { data: { session: newSession } } = await supabase.auth.getSession()
        userEmail = newSession?.user?.email
      }

      if (!userEmail) {
        console.error('Unable to get user email after password update')
        toast({
          title: 'Error',
      ...
      console.log('✅ Supabase password updated successfully for:', userEmail)

      // Sync password to MongoDB
      try {
        console.log('Syncing password to MongoDB for:', userEmail)
        const syncResponse = await fetch('/api/auth/sync-password-from-supabase', {
          method: 'POST',
        ...
        if (!syncResponse.ok) {
          console.error('❌ Failed to sync password to MongoDB:', syncResult)
          toast({
            title: 'Password Updated in Supabase',
            description: `Your password has been updated in Supabase, but there was an issue syncing to the database: ${syncResult.message || 'Unknown error'}. Please contact support if you cannot log in.`,
            variant: 'default',
          })
        } else {
          console.log('✅ Password synced to MongoDB successfully')
          toast({
        ...
        console.error('❌ Error syncing password to MongoDB:', syncError)
        toast({
          title: 'Password Updated in Supabase',
          description: 'Your password has been updated in Supabase, but there was an issue syncing to the database. Please contact support if you cannot log in.',
          variant: 'default',
        })
      }
